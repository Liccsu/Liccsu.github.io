...
## [什么是所有权？](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#什么是所有权)
...
> ### [栈（Stack）与堆（Heap）](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#栈stack与堆heap)
>
...
### [所有权规则](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#所有权规则)
...
### [变量作用域](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#变量作用域)
...
### [`String` 类型](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#string-类型)
...
这两个冒号（`::`）是运算符，允许将特定的 `from` 函数置于 `String` 类型的命名空间（namespace）下，而不需要使用类似 `string_from` 这样的名字。在第 5 章的[“方法语法”（“Method Syntax”）](https://www.rustwiki.org.cn/zh-CN/book/ch05-03-method-syntax.html#方法语法)部分会着重讲解这个语法而且在第 7 章的[“路径用于引用模块树中的项”](https://www.rustwiki.org.cn/zh-CN/book/ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html)中会讲到模块的命名空间。
...
### [内存与分配](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#内存与分配)
...
这是一个将 `String` 需要的内存返回给分配器的很自然的位置：当 `s` 离开作用域的时候。当变量离开作用域，Rust 为我们调用一个特殊的函数。这个函数叫做 [`drop`](https://www.rustwiki.org.cn/zh-CN/std/ops/trait.Drop.html#tymethod.drop)，在这里 `String` 的作者可以放置释放内存的代码。Rust 在结尾的 `}` 处自动调用 `drop`。

> 注意：在 C++ 中，这种 item 在生命周期结束时释放资源的模式有时被称作 **资源获取即初始化**（*Resource Acquisition Is Initialization (RAII)*）。如果你使用过 RAII 模式的话应该对 Rust 的 `drop` 函数并不陌生。

这个模式对编写 Rust 代码的方式有着深远的影响。现在它看起来很简单，不过在更复杂的场景下代码的行为可能是不可预测的，比如当有多个变量使用在堆上分配的内存时。现在让我们探索一些这样的场景。

#### [变量与数据交互的方式（一）：移动](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#变量与数据交互的方式一移动)
...
![String in memory](https://www.rustwiki.org.cn/zh-CN/book/img/trpl04-01.svg)
...
![s1 and s2 pointing to the same value](https://www.rustwiki.org.cn/zh-CN/book/img/trpl04-02.svg)
...
![s1 and s2 to two places](https://www.rustwiki.org.cn/zh-CN/book/img/trpl04-03.svg)
...
![s1 moved to s2](https://www.rustwiki.org.cn/zh-CN/book/img/trpl04-04.svg)
...
#### [变量与数据交互的方式（二）：克隆](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#变量与数据交互的方式二克隆)
...
#### [只在栈上的数据：拷贝](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#只在栈上的数据拷贝)
...
### [所有权与函数](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#所有权与函数)
...
### [返回值与作用域](https://www.rustwiki.org.cn/zh-CN/book/ch04-01-what-is-ownership.html#返回值与作用域)
